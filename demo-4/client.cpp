...
{
    int l_socket = *(int*)socket;
    char file_buffer[128];

    FILE * o_stream;
    o_stream = fopen(input_file, "r");
    fread(&file_buffer, sizeof(char), 128, o_stream);
    
    fclose(o_stream);
    // display on stdout
    int l_len = write( l_socket, file_buffer, strlen(file_buffer) );
    if ( l_len < 0 )
...
int main( int t_narg, char **t_args )
{
    ...
    else
    {
        ...
        int err = write( l_sock_server, str, strlen(str) );
        if ( err < 0 )
            log_msg( LOG_ERROR, "Unable to send data to server." );
        else
            log_msg( LOG_DEBUG, "Sent %d bytes to server.", err );
    ...
    while ( 1 )
    {
        ...
        if ( l_read_poll[ 1 ].revents & POLLIN )
        {
            ...
            if(strncmp(l_buf, "OK\n", 3) == 0)
            {
                ...
                int err = pthread_create( &client_thread, nullptr, thread_function, (void*)&l_sock_server);
                if ( err )
            ...
            FILE * o_stream;
            o_stream = fopen(output_file, "w");

            fprintf(o_stream, "%s", l_buf); // write to file 
